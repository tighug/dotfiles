...
インストールスクリプトは `.zshrc`、`.gitconfig`、`.vimrc`、`.config/`、`.zsh/` をホームディレクトリにシンボリックリンクする。リンク先にシンボリックリンクでない実体が既にある場合はタイムスタンプ付きでバックアップしてから上書きするため、再実行しても安全。注意: `.wezterm.lua` は install スクリプトの管理対象**外**で、Windows ホスト側へ手動で配置する必要がある。
